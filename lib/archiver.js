...
function getEntries(whitelist) {
  if (whitelist.files) {
    // Extract (picks) the property from the whitelist
    const { main, files } = whitelist;
    const whitelistFromSections = [];

    /*
     * It flattens the content of both properties in the "whitelistFromSections" array,
     * and it replaces both "_.reduce" and "_.concat" methods.
     *
     * If the conditional (ternary) operator is too extreme in terms of readability
