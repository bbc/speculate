...
function getExecutableFiles(pkg) {
  const name = pkg.name;
  const executableFiles = getValueFromSpec(pkg, 'executable', []).map((file) => {
  ...
  const serviceProperties = Object.assign(
    {
      ...
      requires: getValueFromSpec(pkg, 'requires', []),
      buildRequires: getValueFromSpec(pkg, 'buildRequires', []),
      postInstallCommands: getValueFromSpec(pkg, 'post', []),
      nodeVersion: getValueFromSpec(pkg, 'nodeVersion'),
      version: pkg.version,
      license: pkg.license,
      prune: getValueFromSpec(pkg, 'prune', true)
    },
